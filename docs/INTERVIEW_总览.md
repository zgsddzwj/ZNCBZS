...
"智能财报助手是一个基于大模型+知识增强的智能财报平台，采用分层架构设计，包含数据层、引擎层和应用层。核心功能包括智能财报分析、报告生成、智能体应用等。技术栈上使用FastAPI作为后端框架，Neo4j存储知识图谱，Milvus存储向量，实现了RAG检索增强生成。项目支持LoRA微调，针对金融领域进行了深度优化。整体完成度约78%，核心功能已实现。"
