...
### MCP架构（Model Context Protocol）
项目采用 **MCP (Model Context Protocol)** 架构，实现标准化的工具调用和数据源访问：
- **MCP Server**: 封装所有工具和数据源，提供标准化接口
- **MCP Client**: 作为Coordinator和Server之间的桥梁
- **工具标准化**: 10+个工具（财报查询、分析、报告生成、智能体等）
- **资源标准化**: 知识库检索、财务数据查询等资源

详细说明请参考 [MCP架构文档](./backend/mcp/README.md)

### 三层架构
- **数据层**：多源数据接入、预处理、知识库存储
- **引擎层**：协同引擎（基于MCP）、大模型服务、知识检索引擎（RAG）
- **应用层**：财报问答、指标分析、报告生成等微服务（通过MCP工具暴露）
